...
| --- | --- | --- | --- | --- |
| send_status | `hold_author_acknowledgement` | `metadata/corresponding_author_signoff_tracker.tsv` | Corresponding-author acknowledgement is incomplete (0/2; proxy working assumption is recorded (2/2)). | Send the sign-off email first and record both confirmations. |
| corresponding_author_acknowledgement | `pending_author_reply` | `metadata/corresponding_author_signoff_tracker.tsv` | 0/2 required corresponding-author confirmations are recorded; proxy working assumption=2/2. | Keep the packet locked until both Yi Miao and Han Yan are confirmed. |
| nature_methods_go_no_go | `conditional_go_after_author_ack` | `results/submission/nature_methods_go_no_go_final.tsv` | Readiness score is 90/100; release status is pass; claim lint violations=0; traceability violations=0; Figure 4 wording freeze present=True; send_status=do_not_send; author_acknowledgement=pending_author_reply (0/2); proxy_working_assumption=2/2. | Corresponding authors must acknowledge the bounded Figure 4 route, then the presubmission inquiry can be reviewed one final time before sending. |
