...
The author-confirmation delivery ZIP is generated by
`python scripts/build_author_confirmation_delivery_bundle.py` and stored at
`output/delivery/RMTGuard_author_confirmation_delivery_2026-05-12.zip`, with a
manifest at `results/submission/author_confirmation_delivery_manifest.tsv` and
a report at `docs/author_confirmation_delivery_bundle.md`. It contains only
author-facing confirmation files and local reply-intake instructions.
Author replies are triaged by `python scripts/build_author_reply_triage.py`,
which reads `metadata/author_reply_evidence/author_reply_log.tsv` and writes
`docs/author_reply_triage.md` plus
`results/submission/author_reply_triage.tsv`. A brief reply such as
`导师：可以` is recorded as preliminary progress but does not by itself unlock
v0.1.1 release or editor-facing submission.
